...
export class TelegramBotService {
  ...
  private userCities: Map<number, string> = new Map<number, string>();
  ...
  private registerCommands() {
    ...
    this.bot.onText(/\/subscribe/, async (msg) => {
      ...
      if (existingUser) {
        ...
        // Ask the user for their preferred city
        this.bot.sendMessage(chatId, 'Please enter your preferred city:');

        // Wait for the user's response
        this.bot.once('text', async (cityMsg) => {
          const userCity = cityMsg.text.trim();

          // Set the user's preferred city in the map
          this.userCities.set(chatId, userCity);
  ...
  private async sendWeatherUpdate(chatId: number) {
    ...
    const CITY = this.userCities.get(chatId);
